...
import { updateServerInfo } from "@/api/api.js"; //编辑套餐
...
export default {
  ...
  methods: {
    ...
    queryInfo() {
      ...
      getServerInfo(params).then(res => {
        ...
        if (res.data.code === 200) {
          ...
          this.propsParams = dataObj;
          console.log(dataObj.cardFromId)
          this.propsParams.cardFromId = JSON.parse(dataObj.cardFromId)
          
          console.log(this.propsParams.cardFromId)
          this.propsParams.netType = JSON.parse(dataObj.netType)
          this.flowInfosList = [];
          let arr = JSON.parse(this.propsParams.flowInfo);
          arr.forEach(item1 => {
            this.flowInfos.forEach(item2 =>{
              if(item1 === item2.value){
                this.flowInfosList.push(item2.label);
              }
            })
          });
          this.propsParams.isDisable = this.propsParams.isDisable ? "1" : "0";
        }
      });
    },
    ...
    handleSubmit() {
      let arr = [];
      // console.log(this.flowInfos)
      this.flowInfos.forEach(item1 => {
      ...
      this.propsParams.flowInfo = JSON.stringify(arr);
      this.propsParams.cardFromId = JSON.stringify(this.propsParams.cardFromId)
      this.propsParams.netType = JSON.stringify(this.propsParams.netType)
      let params = {
        ...this.propsParams
      };
      // console.log(params);
      this.$refs.propsParams.validate(valid => {
        if (valid) {
          updateServerInfo(params).then(res => {
            if (res.data.code === 200) {
